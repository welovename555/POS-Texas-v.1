...
function updatePinDisplay() {
    const { pinDots } = getDOMElements();
    pinDots.forEach((dot, index) => {
        if (index < currentPin.length) {
            dot.classList.add('active');
            // เพิ่ม animation ให้กับจุดล่าสุด
            if (index === currentPin.length - 1) {
...
async function handleLogin() {
    ...
    try {
        const userData = await signInWithPin(currentPin);
        if (userData) {
...
export function LoginPage() {
    // รีเซ็ตค่า PIN ทุกครั้งที่ render หน้านี้
    currentPin = '';

    const view = `
        <div class="login-page-background" id="animated-bg"></div>
        <div class="login-form-container">
            <h1 class="login-title">AUTHENTICATION</h1>
            <p class="login-subtitle">กรุณายืนยันตัวตนด้วยรหัสพิน</p>
